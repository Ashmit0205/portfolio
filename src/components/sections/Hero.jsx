import { useState, useEffect } from 'react';
import { motion } from 'framer-motion';
import { ArrowDown, Download } from 'lucide-react';
import BlurText from '../bits/BlurText';
import Aurora from '../bits/Aurora';
import Magnet from '../bits/Magnet';
import HeroSticker from '../ui/HeroSticker';

const GithubIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" />
  </svg>
);

const LinkedinIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
  </svg>
);

const TYPING_WORDS = [
  'APIs & Automation Pipelines',
  'Systems & Architecture',
  'C • C++ • Python • JavaScript',
  'Computer Vision & Machine Learning'
];

export default function Hero({ theme = 'dark' }) {
  const [typingText, setTypingText] = useState('');
  const [wordIdx, setWordIdx] = useState(0);
  const [charIdx, setCharIdx] = useState(0);
  const [isDeleting, setIsDeleting] = useState(false);

  useEffect(() => {
    const currentWord = TYPING_WORDS[wordIdx];
    let speed = isDeleting ? 40 : 85;
    if (!isDeleting && charIdx === currentWord.length) speed = 2000;
    else if (isDeleting && charIdx === 0) speed = 500;

    const timer = setTimeout(() => {
      if (!isDeleting && charIdx === currentWord.length) {
        setIsDeleting(true);
      } else if (isDeleting && charIdx === 0) {
        setIsDeleting(false);
        setWordIdx((i) => (i + 1) % TYPING_WORDS.length);
      } else {
        setCharIdx((i) => i + (isDeleting ? -1 : 1));
        setTypingText(currentWord.substring(0, charIdx + (isDeleting ? -1 : 1)));
      }
    }, speed);
    return () => clearTimeout(timer);
  }, [charIdx, isDeleting, wordIdx]);

  const isLight = theme === 'light';

  return (
    <section className="hero container" id="home" style={{ position: 'relative', overflow: 'visible' }}>
      {/* React Bits: Aurora Shader Background */}
      <Aurora
        colorStops={isLight ? ['#4f46e5', '#0891b2', '#059669'] : ['#6366f1', '#06b6d4', '#10b981']}
        amplitude={0.9}
        blend={isLight ? 0.2 : 0.3}
        timeScale={0.3}
      />

      <motion.div
        className="hero-content"
        initial={{ opacity: 0, y: 30 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
        style={{ position: 'relative', zIndex: 5 }}
      >
        {/* React Bits: Blur Text Staggered Letter Reveal Headline */}
        <h1
          style={{
            fontSize: 'clamp(1.75rem, 7.5vw, 4.4rem)',
            fontWeight: 900,
            letterSpacing: '-0.03em',
            lineHeight: 1.05,
            margin: '0.4rem 0 0.8rem 0',
            fontFamily: "'Outfit', sans-serif"
          }}
        >
          <BlurText
            text="ASHMIT GUPTA"
            animateBy="letters"
            delay={45}
            direction="top"
            className="hero-name-gradient"
          />
        </h1>

        <div className="hero-title" style={{ margin: '0 0 1rem 0', fontWeight: 600 }}>
          Building software, automation, & systems that turn ideas into reality.
        </div>

        <h2 className="hero-subtitle">
          Focusing on <span id="typing-text">{typingText}</span><span className="cursor">|</span>
        </h2>

        <p className="hero-description">
          I am a computer science student and developer interested in creating practical software. My development journey progresses from low-level algorithms and computer vision experiments to platform systems and automated, API-driven workflows.
        </p>

        <div className="hero-ctas">
          {/* React Bits: Magnet CTA Buttons */}
          <Magnet padding={50} magnetStrength={3}>
            <a href="#journey" className="btn btn-primary" id="hero-view-work">
              <span>View Project Journey</span>
              <ArrowDown size={16} className="btn-icon" />
            </a>
          </Magnet>

          <Magnet padding={50} magnetStrength={3}>
            <a href="#" className="btn btn-secondary" id="download-resume-btn">
              <Download size={16} />
              <span>Download Resume</span>
            </a>
          </Magnet>

          <Magnet padding={40} magnetStrength={2.5}>
            <a href="https://github.com/Ashmit0205" target="_blank" rel="noreferrer" className="btn-icon-link" id="hero-github-link" aria-label="GitHub Profile">
              <GithubIcon />
            </a>
          </Magnet>

          <Magnet padding={40} magnetStrength={2.5}>
            <a href="https://www.linkedin.com/in/ashmit-gupta-386560343/" target="_blank" rel="noreferrer" className="btn-icon-link" id="hero-linkedin-link" aria-label="LinkedIn Profile">
              <LinkedinIcon />
            </a>
          </Magnet>
        </div>
      </motion.div>

      <motion.div
        className="hero-visual"
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 0.8, delay: 0.2, ease: [0.16, 1, 0.3, 1] }}
        style={{ position: 'relative', zIndex: 5 }}
      >
        {/* Interactive 3D Cyber Hacker Sticker Graphic */}
        <HeroSticker />
      </motion.div>
    </section>
  );
}
